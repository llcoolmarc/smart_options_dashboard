...
def analyze_habits(journal: list) -> dict:
    ...
    if len(trades_today) > 2:
        messages.append(f"⛔ Discipline AI: {len(trades_today)} trades today — risk of overtrading. Cap at 2 per day.")
    ...
    for i in range(1, len(journal)):
        ...
        if prev.get("pnl", 0) < 0 and curr.get("date") == prev.get("date"):
            revenge_count += 1
    if revenge_count > 0:
        messages.append(f"⚠️ Discipline AI: Detected {revenge_count} revenge trade(s). Wait before re-entering after losses.")

    # ---------------------------
    # Ignoring stop-loss
    # ---------------------------
    stop_loss_violations = sum(1 for t in journal if t.get("pnl", 0) < -t.get("max_loss", -100))
    ...
    for sym, count in symbol_counts.items():
        if count > 5:
            messages.append(f"⚠️ Discipline AI: {count} trades in {sym}. Diversify to reduce symbol risk.")
    ...
    if clean_sessions > 0:
        messages.append(f"✅ Discipline AI: {clean_sessions} session(s) were clean with zero violations. Stay consistent!")
